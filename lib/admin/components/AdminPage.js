...
class AdminPage extends React.Component<Props> {
  ...
  _getMainContent = (isApplicationAdmin: boolean): React.Node => {
    ...
    const restricted = <p className='text-center lead'>Acceso restringido</p>
  ...
  _getLinks = (isApplicationAdmin: boolean): React.Node[] => {
    const links = [this._createLink('/admin/users', 'Administración de usuarios')]
    // Do not show non-appAdmin users these application-level settings
    if (isApplicationAdmin) {
      if (!isModuleEnabled('enterprise')) {
        links.push(this._createLink('/admin/organizations', 'Organizaciones'))
      }
      links.push(this._createLink('/admin/logs', 'Logs'))
