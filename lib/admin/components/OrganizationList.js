...
class OrganizationList extends Component<Props, State> {
  ...
  save = () => {
    ...
    if (settings) {
      ...
      window.alert('Debe proporcionar un nombre a la organización.')
    }
  }
        ...
        header={
          <Row>
            ...
            <Col xs={8} sm={5} md={6}>
              <h5>
                {organization.name}{' '}
                {orgUsers.length ? <BsLabel>{orgUsers.length} usuarios</BsLabel> : null}
              </h5>
              <small />
            </Col>
            <Col xs={12} sm={5} md={5}>
              <Button
                ...
                {this.state.isEditing
                  ? <span><Icon type='remove' /> Cancelar</span>
