...
function setActiveAlert (alertId: string) {
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    if (alert) {
      ...
      console.warn(`Impossible de trouver une alerte avec l’id: ${alertId}`)
    }
  }
}
  ...
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    if (!project) {
      return dispatch(setErrorMessage({
        message: "Impossible de demander des alertes sans projet défini."
      }))
    }
...
export function saveAlert (alert: Alert) {
  return function (dispatch: dispatchFn, getState: getStateFn) {
    const json = {
      ...
      HeaderText: alert.title || 'Nouvelle Alerte',
...
export function onAlertEditorMount (
  ...
  return function (dispatch: dispatchFn, getState: getStateFn) {
    if (!isModuleEnabled('alerts')) {
      console.warn('Le module des alertes est désactivé. Aborting onAlertEditorMount.')
      return
    }
    ...
    dispatch(fetchProjects(true))
      ...
      .then(() => {
        if (
          ...
          console.log('impossible de créer une alerte!')
          browserHistory.push('/alerts')
          return
        }
        if (!alertId) {
...
export function onAlertsViewerMount (
  ...
  return function (dispatch: dispatchFn, getState: getStateFn) {
    if (!isModuleEnabled('alerts')) {
      console.warn('Le module des alertes est désactivé. Aborting onAlertsViewerMount.')
      return
    }
