...
export default class JobMonitor extends Component<Props> {
  ...
  render () {
    ...
    const activeJobsMessage = retired.length && jobs.length === 0
      ? <span data-test-id='all-jobs-completed'>Todos los trabajos completados</span>
      : <span data-test-id='possibly-active-jobs'>
        {`${jobs.length ? jobs.length : 'Sin'} trabajo${!jobs.length || jobs.length > 1 ? 's' : ''}`}
        ...
        <div className='job-monitor'>
                  ...
                  <li key={job.jobId} className='job-container'>
                    ...
                    <div className='job-container-inner'>
                      ...
                      <div className='job-status-message' >
                        {job.status ? job.status.message : 'esperando'}
                      </div>
                    </div>
                  </li>
                )
          ...
          <div style={{ marginTop: 8 }}>
            <Button
              ...
              <Icon type='times-circle' /> Borrar completados
            </Button>
                ...
                ? <OverlayTrigger
                  ...
                  overlay={
                    <Popover
                      ...
                      title={
                        <span>
                          <Icon type='bug' /> ¡Oh, no! Parece que ha ocurrido un error.
                        </span>
                      }>
                      {supportEmail
                        ? <p>
                          Para enviar un informe de error, envíe un correo electrónico con una 
                          captura de pantalla de la ventana de su navegador, el siguiente texto 
                          (URL actual y detalles del error) y una descripción detallada 
                          de los pasos que siguió a <a href={`mailto:${supportEmail}`}>{supportEmail}</a>.
                        </p>
                        : null
                      }
