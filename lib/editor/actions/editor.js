...
export const getFaresV2Query = (component: string, root: string) => `
  ${root} (limit: -1) {
    id
    ${getGraphQLFieldsForEntity(component, true)}
  }
`
    ...
    const query = `
      query ($namespace: String) {
        feed(namespace: $namespace) {
          ...
          ${getFaresV2Query('faremedia', 'fare_media')}
          ${getFaresV2Query('faretransferrule', 'fare_transfer_rule')}
          ${getFaresV2Query('farelegrule', 'fare_leg_rule')}
          ${getFaresV2Query('farelegjoinrule', 'fare_leg_join_rule')}
          ${getFaresV2Query('network', 'network')}
          ${getFaresV2Query('ridercategory', 'rider_category')}
          routes (limit: -1) {
            id
            route_id
            route_short_name
