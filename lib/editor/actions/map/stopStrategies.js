...
export function addStopToPattern (pattern: Pattern, stop: GtfsStop | GtfsLocation, index?: ?number) {
  ...
  return async function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    // FLEX TODO: is this stuff all messed up by moving away from pattern halts?
    ...
    if (typeof index === 'undefined' || index === null || index === patternStops.length) {
      if (hasShapePoints) {
        ...
        if (patternStops.length === 2 && followStreets) {
          ...
          const previousStopId = patternStops[patternStops.length - 2].stopId || patternStops[patternStops.length - 2].locationId || patternStops[patternStops.length - 2].locationGroupId
          const stops = getTableById(data.tables, 'stop')
          const locations = getTableById(data.tables, 'location')
      ...
      if (hasShapePoints) {
        ...
        try {
          ...
          const addedControlPoint = newControlPoint(
            ...
            insertPoint,
            {
              stopId: stop.stop_id || stop.location_group_id || stop.location_id,
      ...
      if (hasShapePoints) {
        ...
        const controlPoint = newControlPoint(0, stopToPoint(stop), {
          stopId: stop.stop_id || stop.location_id || stop.location_group_id,
          pointType: POINT_TYPE.STOP // 2
        })
