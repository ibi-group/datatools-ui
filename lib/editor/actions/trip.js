...
export function fetchTripsForCalendar (
  ...
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    if (!namespace) throw new Error('El namespace del editor no está definido!')
    ...
    return dispatch(fetchGraphQL({
      ...
      errorMessage: 'No se pudieron buscar viajes para el patrón'
    }))
  ...
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    return dispatch(secureFetch(createUrl, 'post'))
      ...
      .then(trips => {
        // console.log(trips)
        if (errorCount) {
          dispatch(setErrorMessage({message: `Error desconocido al guardar viajes. No se pudo guardar ${errorCount} viajes`}))
        }
        dispatch(fetchTripsForCalendar(feedId, pattern, calendarId, true))
        return errorIndexes
      })
  ...
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    return dispatch(secureFetch(url, 'delete'))
      ...
      .then(trips => {
        // console.log(trips)
        if (errorCount) {
          dispatch(setErrorMessage({message: `Se encontró un error al eliminar viajes. No se pudo eliminar ${errorCount} viajes`}))
        }
        dispatch(fetchTripsForCalendar(feedId, pattern, calendarId, true))
      })
  }
...
export function fetchCalendarTripCountsForPattern (
  ...
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    return dispatch(fetchGraphQL({
      ...
      errorMessage: 'No se pudo obtener el conteo de viajes para el patrón'
    }))
...
export function fetchTripCounts (feedId: string) {
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    return dispatch(fetchGraphQL({
      query,
      variables: {namespace},
      errorMessage: 'No se pudo obtener el conteo de viajes.'
    }))
