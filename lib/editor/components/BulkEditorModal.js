...
export default class BulkEditorModal extends Component<Props, State> {
  ...
  handleBulkEdit = () => {
    ...
    if (invalidFilters.length > 0) {
      return window.alert(`Por favor, elija un operador para los filtros: ${invalidFilters.join(', ')}`)
    }
  ...
  handleSnapshotClick = () => {
    const name = window.prompt('Introduce un nombre para el snapshot', formatTimestamp())
    if (name) this.props.createSnapshot(this.props.feedSource, name)
    else if (name === '') window.alert('Debe proporcionar un nombre al snapshot!')
  }
  ...
  render () {
    ...
    if (!currentTable) throw new Error(`No se encontró ninguna tabla para ${activeComponent}`)
    ...
    return (
      ...
      >
        <Header>
          <Title>Editor para {currentTable.name}</Title>
        </Header>
        <Body>
          <p>
            Utilice este formulario para editar todos los {activeComponent}s 
            contenidos en este feed GTFS. También puede aplicar un conjunto de 
            ediciones solo a un subconjunto de {activeComponent} usando 
            los filtros a continuación.
          </p>
          <ControlLabel>Agregar campo para actualizar:</ControlLabel>
          <FormControl
            value=''
          ...
          >
            <option value=''>Seleccionar campo para actualizar</option>
            {
              currentTable.fields
          ...
          {patch
            ? Object.keys(patch).map(key => {
              ...
              if (!field) throw new Error(`No se puede encontrar el campo para la clave: ${key}`)
              return (
                <PatchField
                  key={field.name}
          ...
          <hr />
          <ControlLabel>Agregar campo para filtrar:</ControlLabel>
          <FormControl
            componentClass='select'
            value=''
            style={{ display: 'inline', width: '200px', marginLeft: '10px' }}
          ...
          <div
            ...
            style={{ margin: '10px' }}>
            Los cambios se aplican a {hasFilters ? 'registros donde:' : 'todos los registros.'}
          </div>
          {filter
          ...
          <Alert bsStyle='warning' style={{ marginTop: '20px' }}>
            <strong>Advertencia:</strong> La edición modificará todos los registros 
            (o aquellos registros que coincidan con el filtro), esta operación no se puede deshacer!
            <hr />
            Se recomienda crear primero un snapshot del estado actual de los datos
            en el editor antes de aplicar una edición masiva. Un snapshot toma
            solo unos segundos y le permite revertir sus cambios.
            <Button
              onClick={this.handleSnapshotClick}
              style={{ marginTop: '10px' }}
              block>
              <Icon type='camera' /> Crear snapshot primero
            </Button>
          </Alert>
        </Body>
        <Footer>
    ...
    return (
      <div className='col-xs-12'>
        ...
        <Button
          ...
          title='Eliminar forma para patrón'
    ...
    return (
      <div>
        ...
        <FormControl
          ...
          componentClass='select'
        >
          <option disabled value=''>escoger operador</option>
