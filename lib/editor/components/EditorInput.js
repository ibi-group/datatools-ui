...
type Props = {
  ...
  isNotValid: boolean,
  onChange?: any => void,
  preventUpdate?: ?() => void, // Optional, used for BookingRules
...
const isPriorRuleType = (ruleType) => ['1', '2'].includes(ruleType)
...
export default class EditorInput extends React.Component<Props> {
  ...
  _processFieldChange = (val: any) => {
    ...
    // $FlowFixMe: flow doesn't recognize activeComponent check
    if (activeComponent === 'bookingrule' && field.name === 'booking_type' && activeEntity.booking_type && isPriorRuleType(activeEntity.booking_type) && !isPriorRuleType(val)) {
      // $FlowFixMe need a BookingRule entity check
      const priorFieldsEmpty = !Object.keys(activeEntity).some(key => key.includes('prior') && activeEntity[key]) // 0's are strings here so truthy check is fine.
      if (!priorFieldsEmpty) {
        preventUpdate && preventUpdate() // Set the BookingRuleForm's state to know that the update was prevented.
        toast.error(
          this.messages('realtimeBookingTypeErrorToast'),
          {
            position: 'top-right',
            autoClose: 6000,
  ...
  render () {
    ...
    switch (field.inputType) {
      ...
      case 'GTFS_ZONE':
        if (!zoneOptions) return null // Explicit check for zone options to make flow happy
        return (
          <FormGroup {...formProps}>
            {basicLabel}
            <ZoneSelect
      ...
      case 'DROPDOWN':
        ...
        const { updatePrevented } = this.props
        const disableRealtimeBookingRule = activeComponent === 'bookingrule' && field.name === 'booking_type' && isPriorRuleType(currentValue)

        const DefaultOption = (props) => (
          <option key={props.value} value={props.value} {...props}>
          ...
          <FormGroup
            ...
            >
              ...
              {field.options && field.options.map(o => {
                if (disableRealtimeBookingRule && updatePrevented && !isPriorRuleType(o.value)) {
                  // If we're on the option for booking_type and our disable condition is met, return a special option with overlay
                  // to explain why the option is disabled (user must clear prior_notice fields).
                  return (
                    <OverlayTrigger>
                      <DefaultOption
                        disabled
                        text={o.text}
                        title={this.messages('realtimeBookingTypeErrorTooltip')}
                        value={o.value}
                      />
                    </OverlayTrigger>
                  )
                }
                return <DefaultOption text={o.text} value={o.value} />
              })}
            </FormControl>
          </FormGroup>
        )
