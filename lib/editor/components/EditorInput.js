  ...
  render () {
    ...
    switch (field.inputType) {
        ...
        if (field.name.indexOf('_branding_url') > -1) {
          ...
          elements.push(
            <FormGroup
              ...
              <ControlLabel>Cargar {activeComponent} recurso de marca</ControlLabel>
              <Dropzone
                accept='image/*'
                multiple={false}
                onDrop={this._uploadBranding}>
                <div style={{marginBottom: '10px'}}>Suelte la imagen {activeComponent} aquí, o haga clic para seleccionar la imagen para cargar.</div>
                {url
                  ? <img
                    alt={field.name}
      ...
      case 'DATE':
        ...
        if (!currentValue) {
          dateTimeProps.defaultText = 'Por favor seleccione una fecha'
        }
        return (
          <FormGroup {...formProps}>
            {basicLabel}
          ...
          <span>
            <span>
              {editorField === 'monday' // insert label for entire category on first checkbox
                ? <div className='col-xs-12'><ControlLabel>Dias de servicio</ControlLabel></div>
                : null
              }
            </span>
            ...
            >
              ...
              {options.findIndex(option => option.value === '') === -1 && (
                <option disabled value=''>
                  {field.required ? '-- Seleccione una opción --' : '(opcional)' }
                </option>
              )}
