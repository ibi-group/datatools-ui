...
export default class FeedInfoPanel extends Component<Props, State> {
  ...
  _onSelectSnapshot = (key: string) => {
    ...
    snapshot && this.props.showConfirmModal({
      title: `Restaurar ${key}?`,
      body: `¿Estás seguro de que quieres restaurar este snapshot?`,
      onConfirm: () => {
    ...
    return (
      <div>
        ...
        <div style={panelStyle}>
          ...
          <ButtonGroup>
            {/* Hide toolbar toggle */}
            <OverlayTrigger placement='top' overlay={<Tooltip id='hide-tooltip'>{toolbarVisible ? 'Esconder barra de herramientas' : 'Mostrar barra de herramientas'}</Tooltip>}>
              <Button
            ...
            <DropdownButton
              dropup
              title={<span title={`Editando ${feedSource && feedSource.name}`}>Editando {feedName}</span>}
              id='navigation-dropdown'
              onSelect={this._onNavigate}>
              <MenuItem eventKey='1'><Icon type='reply' /> Regresar al proyecto</MenuItem>
              <MenuItem eventKey='2'><Icon type='reply' /> Regresar al feed origen</MenuItem>
            </DropdownButton>
            <Button
              ...
              {GTFS_ICONS.map(c => {
                ...
                return (
                  <MenuItem
                    key={c.id}
                    eventKey={c.id}>
                    <Icon type={c.icon} /> Agregar {componentToText(c.id)}
                  </MenuItem>
                )
              })}
            </DropdownButton>
            {/* Snapshot dropdown */}
            {
              <Dropdown
                ...
                <OverlayTrigger
                  placement='top'
                  overlay={<Tooltip id='snapshot-tooltip'>Crear snapshot</Tooltip>}>
                  <Button
                    bsStyle='primary'
                    ...
                    ? feedSource.editorSnapshots.map(snapshot => {
                      return (
                        <MenuItem
                          ...
                          <Icon type='reply' /> Revertir a {snapshot.name}
                        </MenuItem>
                      )
                    })
                    : <MenuItem
