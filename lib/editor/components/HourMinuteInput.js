...
const totalSecToMinComponent = (seconds: number) => Math.floor(seconds / 60 % 60)
const totalSecToHourComponent = (seconds: number) => Math.floor(seconds / 3600)
...
export default class HourMinuteInput extends Component<Props, State> {
  ...
  updateStateFromSeconds (prevProps?: Props) {
    const {seconds} = this.props
    if ((!prevProps || seconds !== prevProps.seconds) && seconds !== null && seconds !== undefined) {
      const hours = totalSecToHourComponent(seconds)
      const minutes = totalSecToMinComponent(seconds)
      this.setState({minutes, hours})
    }
  }

  componentDidUpdate = (prevProps: Props) => this.updateStateFromSeconds(prevProps)
  componentDidMount = () => this.updateStateFromSeconds()
