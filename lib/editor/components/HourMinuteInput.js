...
const totalSecToMin = (seconds: number) => Math.floor(seconds / 60 % 60)
const totalSecToHour = (seconds: number) => Math.floor(seconds / 3600)
...
export default class HourMinuteInput extends Component<Props, State> {
  ...
  updateStateFromSeconds (prevProps?: Props) {
    ...
    if ((!prevProps || seconds !== prevProps.seconds) && seconds !== null && seconds !== undefined) {
      const hours = totalSecToHour(seconds)
      const minutes = totalSecToMin(seconds)
