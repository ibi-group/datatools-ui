...
import {getEntityName, getKeyForId} from '../util/gtfs'
...
type Props = {
  ...
  style?: Style,
  /**
   * Toggle tableSelect when the above entities array is a list of component ids defined in the constant COMPONENT_LIST in editor/util/gtfs.js.
   * This will use the getKeyForId function to render the label as the component's tableName.
  */
  tableSelect?: boolean,
  value?: any
}
...
export default class VirtualizedEntitySelect extends Component<Props, State> {
  ...
  _entityToOption = (entity: any) => {
    const {entityKey, tableSelect} = this.props
    return {
      value: tableSelect ? entity : entity[entityKey],
      label: tableSelect ? getKeyForId(entity, 'tableName') : getEntityName(entity) || '[Unnamed]',
      entity
    }
  }
