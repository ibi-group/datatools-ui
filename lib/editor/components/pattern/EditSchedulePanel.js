...
export default class EditSchedulePanel extends Component<Props> {
  ...
  _onChangeUseFrequency = (key: string) => {
    ...
    showConfirmModal({
      ...
      body: `Êtes-vous sûr de vouloir utiliser ${key} pour cette circulation ?
      Les trajets créés en utilisant ${unselectedOption} seront perdus.`,
      onConfirm: () => {
  ...
  render () {
    ...
    return (
      <div>
        <h4 className='line'>
          Fiches horaires {`(${activePatternTripCount} trajet${activePatternTripCount !== 1 ? 's' : ''})`}
        </h4>
        {!activePatternTripCount
          ? <small className='text-warning'>
            <Icon type='exclamation-triangle' />{' '}
            Pas de trajets en service pour cette circulation.
          </small>
          : null
        }
        ...
        <Button
          ...
          title={editSchedulesDisabled
            ...
            : `Ajouter ou modifier des trajets pour ${name}`}
        >
