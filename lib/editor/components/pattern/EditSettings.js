...
export default class EditSettings extends Component<Props, State> {
  ...
  render () {
    ...
    const SETTINGS = [
      {type: 'followStreets', label: 'Ajustar a las calles'},
      {type: 'avoidMotorways', label: 'Evitar autopistas en la ruta'},
      {type: 'hideStopHandles', label: 'Ocultar controladores de parada'},
      {type: 'hideInactiveSegments', label: 'Ocultar segmentos inactivos'},
      {type: 'showStops', label: 'Mostrar paradas'},
      {type: 'showTooltips', label: 'Mostrar tooltips'}
    ]
    ...
    return (
      <div>
        ...
        <ControlLabel><small>Modo de edición de forma</small></ControlLabel>
        <FormControl
        ...
        {onMapClick.includes('ADD_')
          ? <Alert bsStyle='warning' style={{marginTop: '30px'}}>
            <small>
              ...
              {' '}
              Este modo de edición crea nuevas paradas. A menos que no haya paradas cercanas, no se recomienda este modo.
            </small>
          </Alert>
          : null
        }
