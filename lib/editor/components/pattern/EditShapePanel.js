...
export default class EditShapePanel extends Component<Props> {
  ...
  async drawPatternFromStops (pattern: Pattern, stopsCoordinates: Array<LatLng>): Promise<any> {
    ...
    let patternSegments = []
    if (editSettings.present.followOption !== 'NONE') {
      patternSegments = await getPolyline(stopsCoordinates, true, editSettings.present.avoidMotorways, editSettings.present.followOption)
    } else {
