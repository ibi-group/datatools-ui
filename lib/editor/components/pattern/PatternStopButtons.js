...
export default class PatternStopButtons extends Component<Props> {
  ...
  _onClickRemove = () => {
    ...
    if (
      window.confirm(
        `Êtes-vous sûr de vouloir supprimer ${stop.stop_name} (${stop.stop_id}) en tant qu'arrêt #${index + 1}?`
      )
    ) {
  ...
  render () {
    ...
    return (
      <ButtonGroup
        ...
        <OverlayTrigger
          overlay={<Tooltip id='edit-stop-tooltip'>Editer l'arrêt</Tooltip>}
        >
          <Button
            bsSize={size}
        ...
        <OverlayTrigger
          overlay={
            <Tooltip id='remove-stop-tooltip'>Supprimer l'arrêt de la circulation</Tooltip>
          }
        >
          <Button
            bsSize={size}
