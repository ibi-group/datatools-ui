...
export default class PatternStopButtons extends Component<Props> {
  ...
  _onClickRemove = () => {
    ...
    if (
      window.confirm(
        `¿Estás seguro de que te gustaría eliminar ${stop.stop_name} (${stop.stop_id}) como parada de patrón #${index + 1}?`
      )
    ) {
  ...
  render () {
    ...
    return (
      <ButtonGroup
        ...
        <OverlayTrigger
          overlay={<Tooltip id='edit-stop-tooltip'>Editar parada</Tooltip>}
        >
          <Button
            bsSize={size}
        ...
        <OverlayTrigger
          overlay={
            <Tooltip id='remove-stop-tooltip'>Eliminar parada del patrón</Tooltip>
          }
        >
          <Button
            bsSize={size}
