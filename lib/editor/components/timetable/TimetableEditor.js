  ...
  constructNewRow = (toClone: ?Trip = null, tripSeriesStartTime: ?number, autoTripId: ?string, autoBlockId: ?string): ?Trip => {
    ...
    if (toClone) {
      ...
      for (let i = 0; i < patternHalts.length; i++) {
        ...
        // $FlowFixMe Flow doesn't understand our flex check
        objectPath.set(newRow, `stopTimes.${i}.shapeDistTraveled`, patternHaltIsStop(halt) ? halt.shapeDistTraveled : null)
