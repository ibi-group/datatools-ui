...
export default class TimetableEditor extends Component<Props, State> {
  ...
  constructNewRow = (toClone: ?Trip = null, tripSeriesStartTime: ?number, autoTripId: ?string, autoBlockId: ?string): ?Trip => {
    ...
    if (toClone) {
      ...
      for (let i = 0; i < activePattern.patternStops.length; i++) {
        const halt = activePattern.patternStops[i]
        ...
        if (halt.stopId !== null) {
          ...
          const nextHalt = i + 1 !== activePattern.patternStops.length ? activePattern.patternStops[i + 1] : null

          if (halt.locationId !== null) {
            objectPath.set(newRow, `stopTimes.${i}.locationId`, halt.locationId)
          }
          if (halt.locationGroupId !== null) {
            objectPath.set(newRow, `stopTimes.${i}.locationGroupId`, halt.locationGroupId)
          }
          objectPath.set(newRow, `stopTimes.${i}.startPickupDropOffWindow`, cumulativeTravelTime)
          objectPath.set(newRow, `stopTimes.${i}.endPickupDropOffWindow`, cumulativeTravelTime + halt.defaultDwellTime)

          // We don't want to increment for intra-zonal travel times.
          if (halt.locationId !== (nextHalt ? nextHalt.locationId : '')) cumulativeTravelTime += +halt.defaultDwellTime
        }
        // }
        // if (stop.timepoint === null || stop.timepoint) {
        objectPath.set(newRow, `stopTimes.${i}.departureTime`, halt.stopId !== null ? cumulativeTravelTime : null)
  ...
  removeSelectedRows = () => {
    ...
    showConfirmModal({
      ...
      body: this.messages(selectedCount > 1 ? 'deleteSelectedTrips' : 'deleteSelectedTrip').replace('%count%', selectedCount.toString()),
    ...
    return (
      <div
        ...
        <TimetableHeader
          ...
          setOffset={setOffset}
          setScrollIndexes={setScrollIndexes}
          ...
          tripCounts={tripCounts}
          tripValidationErrors={tripValidationErrors} />
        {activeSchedule
          ? <Timetable
