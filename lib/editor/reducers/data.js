...
const data = (state: DataState = defaultState, action: Action): DataState => {
  switch (action.type) {
    ...
    case 'RECEIVE_BASE_GTFS': {
      ...
      if (!feed) {
        console.warn('Impossible de récupérer le GTFS !')
        return state
      }
      ...
      if (feed.feed_info.length === 0) {
        console.warn(`Aucune information de flux trouvé. Ajout d'informations de flux avec des valeurs nulles.`)
    ...
    case 'UNDO_TRIP_PATTERN_EDITS': {
      ...
      if (controlPoints && patternSegments) {
        ...
        if (!subEntity) {
          console.warn('Pas de tracé de circulation trouvé', state.active)
          return state
        }
        ...
        if (!entity) {
          console.warn('Aucune entité trouvée', state.active)
          return state
        }
    ...
    case 'RESET_ACTIVE_GTFS_ENTITY': {
      ...
      if (!stateEntity) {
        console.warn('Entité non trouvée', state.active)
        return state
      }
