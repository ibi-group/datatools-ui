...
export const getActivepatternLocationGroupStops = createSelector(
  [
    getActivePattern,
    getLocationGroups
  ],
  (pattern, locationGroups) => {
    if (!pattern || !locationGroups || !pattern.patternLocationGroupStops) return null
    const patternLocationGroupStops = pattern.patternLocationGroupStops.map(pl => {
