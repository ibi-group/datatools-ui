...
import {getTableById, getEntityName, FARESV2_COMPONENTS} from '../util/gtfs'
...
export const getFaresV2AggregateStatus: AppState => Array<FaresV2FileStatus> = createSelector(
  [
    (state: AppState) => state.editor.data.tables
  ],
  (tableData) => {
    return FARESV2_COMPONENTS.map(component => {
      const entities = getTableById(tableData, component)
      ...
      const invalidRowCount = entities.reduce((count, entity) => {
        const errors = getFieldErrors(table.fields, entity, entities, tableData)
...
): Array<EditorValidationIssue | false> {
  if (!entity || !fields) {
    return []
  }
  const currentEntity: Entity = entity
  return fields
    .map(field =>
      validate(field, currentEntity[field.name], entities, currentEntity, tableData)
    )
...
export const getValidationErrors: AppState => Array<EditorValidationIssue | false> = createSelector(
  ...
  (component: ?string, entity: ?Entity, entities: ?Array<Entity>, tableData) => {
    ...
    const fields = table ? table.fields : undefined
    const errors = getFieldErrors(fields, entity, entities, tableData)
