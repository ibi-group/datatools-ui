      ...
      fare.fare_rules && fare.fare_rules.forEach((rule, i) => {
        if (
          ...
          errors.push({
            ...
            invalid: true,
            reason: 'Uno de route_id, origin_id, destination_id o contains_id debe tener un valor especificado.'
          })
        }
      })
    }
    ...
    const list =
      entities && entities.length
        ? entities.map((entity, index) => {
          ...
          const name = getEntityName(entity) || '[Sin nombre]'
...
function getControlPointsFromShapePoints (shapePoints: Array<ShapePoint>) {
  if (shapePoints.length < 2) {
    throw new Error('Los puntos de forma deben contener dos o más coordenadas.')
  }
  const controlPoints = []
    ...
    if (i > 0) {
      ...
      if (i < patternStops.length - 1) {
        ...
        if (remainingLine.geometry.coordinates.length < 2) {
          throw new Error(
            `Línea restante después de la parada del patrón #${i}/${patternStops.length -
              1} tiene menos de dos coordenadas`
          )
        }
      }
      patternSegments.push(lineSegment.geometry.coordinates)
...
export function sliceAtPoint (
  ...
  if (!alongPoint && !lengthInMeters && lengthInMeters !== 0) {
    throw new Error(
      'Debe proporcionar el argumento alongPoint o un número positivo para lengthInMeters'
    )
  } else if (!alongPoint) {
