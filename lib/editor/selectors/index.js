...
import { validate } from '../util/validation/validation'
...
export const getActivePatternLocations = createSelector(
  ...
  (pattern, locations) => {
    if (!pattern || !locations || !pattern.patternStops) return null
    const patternLocations = pattern.patternStops.map(pl => {
...
export const getActivePatternLocationGroups = createSelector(
  ...
  (pattern, locationGroups) => {
    if (!pattern || !locationGroups || !pattern.patternStops) return null
    const patternLocationGroupStops = pattern.patternStops.map(pl => {
...
export const getControlPoints = createSelector(
  [
    ...
    getActivePatternLocations,
    getActivePatternLocationGroups,
  ...
  (
    ...
    if (hasShapePoints) {
      ...
      if (hasPatternStops) {
        ...
        try {
          const result = generateControlPointsFromPatternStops(patternStops, activePatternStops, activePatternLocations, activePatternLocationGroups)
          return result
        } catch (e) {
...
function addPatternStopsToShapePoints (
  ...
  for (var i = 0; i < patternStops.length; i++) {
    ...
    const stop =
      (activePatternStops &&
        activePatternStops.find((st) => st !== undefined && st.stop_id === stopId)) ||
      (activePatternLocations &&
        activePatternLocations.find((l) => l !== undefined && l.location_id === locationId)) ||
      (activePatternLocationGroups &&
        activePatternLocationGroups.find(
          (lg) => lg !== undefined && lg.location_group_id === locationGroupId
        ))
    if (!stop) {
      console.warn(`No stop found for pattern stop ID: ${stopId || '(no stop id found)'}`, activePatternStops)
      continue
    }
    ...
    if (i > 0) {
      ...
      if (!!stop.stop_lon && !!stop.stop_lat && (sliceDistance <= 0 || isNaN(sliceDistance))) {
