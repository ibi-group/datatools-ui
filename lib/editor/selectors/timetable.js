  ...
  if (isTimeFormat(col.type)) {
    ...
    if (typeof val === 'number' && val >= 0) {
      ...
      return isInvalid
        ? {
          field: col.key,
          invalid: true,
          reason: 'El valor del tiempo no debe ser menor que el tiempo anterior.'
        }
        : null
    } else {
      if (col.type === 'TIME') {
        ...
        return {
          field: col.key,
          invalid: true,
          reason: `La entrada Frequency debe tener ${type} tiempo definido.`
        }
      }
      if (
        ...
        return {
          field: col.key,
          invalid: true,
          reason: 'Las horas de la primera y última parada deben tener definidas las horas de llegada y salida.'
        }
      }
      ...
      return typeof companionTime === 'number'
        ? {
          field: col.key,
          invalid: true,
          reason: 'El tiempo debe definirse si el tiempo complementario tiene un valor.'
        }
        : null
    }
  } else {
    ...
    if (col.key === 'tripId' && val && tripIds.indexOf(val) !== -1) {
      ...
      return {
        field: col.key,
        invalid: true,
        reason: 'El ID de viaje debe ser único para el conjunto de datos.'
      }
      ...
      if (val <= 0 || !Number.isInteger(val)) {
        return {
          field: col.key,
          invalid: true,
          reason: 'Los segundos Headway deben ser un número entero positivo.'
        }
      } else if (val > MAX_HEADWAY_SECONDS) {
        return {
          field: col.key,
          invalid: true,
          reason: `Los segundos Headway no debe de ser mayor a ${MAX_HEADWAY_SECONDS}.`
        }
      }
    }
  }
