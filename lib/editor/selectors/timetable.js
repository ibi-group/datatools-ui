    ...
    if (pattern && pattern.patternStops) {
      if (!pattern.useFrequency) {
        pattern.patternStops.map((ps, index) => {
          const isLocation = ps.locationId !== null || ps.locationGroupId !== null
          ...
          const TIME_WIDTH = 80
          columns.push({
            name: abbreviatedStopName || 'Pickup/Dropoff Window Start',
            title: `${stopName ? stopName + ' ' : ''}(${stopId})`,
            width: TIME_WIDTH,
          ...
          columns.push({
            name: 'Pickup/Dropoff Window End',
