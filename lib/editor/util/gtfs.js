...
export const FARESV2_COMPONENTS = [
  'area',
  'faremedia',
  'fareproduct',
  'faretransferrule',
      ...
      route.tripPatterns.map(pattern => {
        ...
        if (stops && pattern.patternStops && pattern.patternStops.length > 0) {
          ...
          const locationCoordinates = pattern.patternStops.filter(ps => ps.locationId !== null)
            ...
            // $FlowFixMe flow doesn't know about flatmap
...
export function getEntityName (entity: any): string {
  ...
  if (typeof entity.route_id !== 'undefined') {
    ...
    nameKey = 'agency_name'
  } else if (typeof entity.stop_id !== 'undefined' && typeof entity.location_group_id === 'undefined') { // Location Groups have a stop id, but shouldn't be named as a location group
    nameKey = 'stop_name'
  } else if (typeof entity.location_id !== 'undefined' && typeof entity.location_group_id === 'undefined') { // Location Groups have a location id, but shouldn't be named as a location group
...
/** Extracts a trip pattern array (or empty array) from an entity, with type checks. */
export function getTripPatterns (entity: Entity): Array<Pattern> {
  if (entity.tripPatterns) {
    const patterns = entity.tripPatterns
...
export function isComponentFaresv2 (component: string): boolean {
  return FARESV2_COMPONENTS.includes(component)
}
