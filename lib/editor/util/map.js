...
export function generateControlPointsFromPatternStops (
  ...
  for (let i = 0; i < patternStops.length; i++) {
    const stop = stops.find(st => st && st.stop_id === patternStops[i].stopId) || locations.find(st => st && st.location_id === patternStops[i].stopId) || locationGroups.find(st => st && st.location_group_id === patternStops[i].stopId)
