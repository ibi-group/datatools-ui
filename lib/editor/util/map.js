...
export function newControlPoint (
  ...
  if (!distance && distance !== 0) {
    throw new Error(
      'la distance doit être définie lors de la création d\'un nouveau point de contrôle'
    )
  }
  return {
    distance,
...
export function getPatternEndPoint (pattern: Pattern, controlPoints?: Array<ControlPoint>): LatLng {
  ...
  throw new Error('Le point de contrôle n\'a pas de coordonnées définies')
}
...
export async function recalculateShape ({
  ...
  if (editType === 'update') {
    ...
    throw new Error('editType invalide. Doit être "update" ou "supprimer".')
  }
  ...
  if (!newSegment || !newSegment.coordinates) {
    ...
    console.warn(
      `Impossible de dessiner entre les points pour le point de contrôle ${index}`,
...
export async function route (from: LatLng, to: LatLng): Promise<any> {
  ...
  if (!r5url) {
    throw new Error('URL r5 url non définie')
  }
...
export function stopToPatternStop (
  ...
  if (!stop.stop_id) {
    throw new Error('ID de l\'arrêt non défini')
  }
  return {
...
export function generateControlPointsFromPatternStops (
  ...
  for (let i = 0; i < patternStops.length; i++) {
    ...
    if (!stop) throw new Error(`Arrêt non trouvé pour le motif à l\'index ${i}.`)
...
export function controlPointsFromSegments (
  ...
  for (let i = 0; i <= patternSegments.length; i++) {
    ...
    if (!coordinate || coordinate.length !== 2) {
      ...
      throw new Error(`Les coordonées du segment index ${i}/${patternSegments.length} n'existent pas`)
    }
...
export function getPatternDistance (
  ...
  if (shape) {
    if (!shape.coordinates) {
      throw new Error('coordonnées de tracé non valides')
    }
    ...
    if (controlPoints.length > 0) {
      try {
        ...
        console.warn('Mauvais point de contrôle. Distance par défaut à zéro', e, controlPoints)
        return 0
      }
    }
  }
...
export function getDistanceScaleFactor (shapePoints: Array<ShapePoint>): number {
  ...
  if (totalShapePointsDistTraveled) {
    ...
    console.warn(`Pas de shape dist traveled value trouvé pour le dernier point. Facteur d'échelle de distance par défaut à 1.`)
    distScaleFactor = 1
  }
