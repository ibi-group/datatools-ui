...
export function stopToPatternStop (
  ...
  if (stop.location_group_id !== undefined && stop.location_group_id !== null) {
    return {
      continuousDropOff: 1,
      continuousPickup: 1,
      defaultDwellTime: 0,
  ...
  if (stop.location_id !== undefined && stop.location_id !== null) {
    return {
      continuousDropOff: 1,
      continuousPickup: 1,
      defaultDwellTime: 0,
      ...
      // $FlowFixMe flow doesn't understand the type check above
  ...
  return {
    continuousDropOff: 1, // FLEX TODO: Check against route attributes
    continuousPickup: 1, // FLEX TODO: Check against route attributes
    defaultDwellTime: 0,
    defaultTravelTime: 0,
    dropOffBookingRuleId: null,
    dropOffType: null,
...
export function generateControlPointsFromPatternStops (
  ...
  for (let i = 0; i < patternStops.length; i++) {
    const stop = stops.find(st => st && st.stop_id === patternStops[i].stopId) || locations.find(st => st && st.location_id === patternStops[i].locationId) || locationGroups.find(st => st && st.location_group_id === patternStops[i].locationGroupId)
    ...
    const controlPoint = newControlPoint(cumulativeDistance, point(coordinate),
      {
        id: i,
        pointType: POINT_TYPE.STOP,
        stopId: patternStops[i].stopId || patternStops[i].locationId || patternStops[i].locationGroupId,
        shapePtSequence: i
      }
    )
...
export function projectStopOntoLine (stop: GtfsStop | GtfsLocation, line: GeoJsonLinestring): {
