...
export function newControlPoint (
  ...
  if (!distance && distance !== 0) {
    throw new Error(
      'el argumento de distancia debe establecerse al crear un nuevo punto de control'
    )
  }
  return {
    distance,
...
export async function constructStop (
  ...
  let stopName = `Nueva parada (${stopId})`
...
export function getPatternEndPoint (pattern: Pattern, controlPoints?: Array<ControlPoint>): LatLng {
  ...
  throw new Error('Al punto de control le faltan las coordenadas')
}
...
export async function recalculateShape ({
  ...
  if (editType === 'update') {
    ...
    throw new Error('EditType no válido. Debe ser "actualizar" o "eliminar".')
  }
...
export async function route (from: LatLng, to: LatLng): Promise<any> {
  ...
  if (!r5url) {
    throw new Error('R5 url no establecida')
  }
...
export function stopToPatternStop (
  ...
  if (!stop.stop_id) {
    throw new Error('No se ha establecido el ID de parada')
  }
  return {
    id: generateUID(),
    stopSequence,
...
export function generateControlPointsFromPatternStops (
  ...
  for (let i = 0; i < patternStops.length; i++) {
    ...
    if (!stop) throw new Error(`No se encontró la parada para la parada del patrón en el índice ${i}.`)
...
export function controlPointsFromSegments (
  ...
  for (let i = 0; i <= patternSegments.length; i++) {
    ...
    if (!coordinate || coordinate.length !== 2) {
      ...
      throw new Error(`Coordenada en índice de segmento ${i}/${patternSegments.length} no existe`)
    }
...
export function getPatternDistance (
  ...
  if (shape) {
    if (!shape.coordinates) {
      throw new Error('Coordenadas de forma no válidas')
    }
