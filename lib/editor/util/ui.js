...
export const GTFS_ICONS = [
  ...
  },
  {
    ...
    icon: 'ticket',
    addable: true,
    title: messages('faresv2.title'),
    label: messages('faresv2.label')
  }
]

// Fares v2 tables are selected through the Fares v2 table selector rather
// than individual sidebar entries, so keep their icons separate from the
// navigation metadata above.
export const FARES_V2_ICONS = [
  {id: 'fareproduct', icon: 'ticket'},
  {id: 'faremedia', icon: 'credit-card'},
  {id: 'faretransferrule', icon: 'exchange'},
  {id: 'farelegrule', icon: 'road'},
  {id: 'farelegjoinrule', icon: 'link'},
  {id: 'ridercategory', icon: 'users'},
  {id: 'area', icon: 'map'},
  {id: 'network', icon: 'sitemap'},
  {id: 'timeframe', icon: 'clock-o'}
]

export function getGtfsIconName (component: string): ?string {
  const icon = FARES_V2_ICONS.find(i => i.id === component) ||
    GTFS_ICONS.find(i => i.id === component)
