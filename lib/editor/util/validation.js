...
export function validate (
  ...
  function emptyFieldValidationIssue (field = name) {
    return validationIssue('El campo obligatorio no debe estar vacío', field)
  }
  ...
  function checkPositiveNumber (): CheckPositiveOutput {
    ...
    if (isNaN(num)) {
      return {
        result: validationIssue('El campo debe ser un número válido')
      }
    }
    ...
    if (num < 0) {
      return {
        result: validationIssue('El campo debe ser un número positivo')
      }
    }
  ...
  function checkGtfsRouteType (): CheckPositiveOutput {
    ...
    if (!isValid) {
      return {
        result: validationIssue('El campo debe ser un tipo de ruta válido')
      }
    }
  ...
  switch (inputType) {
    case 'GTFS_ID':
      ...
      if (
        ...
        return validationIssue('Se requiere identificador si existe más de una agencia')
      }
      if (isRequiredButEmpty) {
        return emptyFieldValidationIssue()
      } else if (isNotUnique) {
        return validationIssue('El identificador debe ser único')
      } else {
        return false
      }
    case 'TEXT':
      if (
        ...
        return validationIssue('El nombre de la parada es obligatorio para los tipos de ubicación de parada, estación y entrada.')
      }
    ...
    case 'URL':
      ...
      if (isRequiredButEmpty) {
        ...
        return validationIssue('El campo debe contener una URL válida.')
      } else {
        return false
      }
    case 'EMAIL':
      ...
      if (isRequiredButEmpty) {
        ...
        return validationIssue('El campo debe contener una dirección de correo electrónico válida.')
      } else {
        return false
      }
    case 'GTFS_ZONE':
    ...
    case 'LATITUDE':
      ...
      if (isNotLat) {
        return validationIssue('El campo debe ser una latitud válida.')
      }
      if (isOptionalAndEmpty && locationType !== null && (typeof locationType === 'number' && locationType <= 2)) {
        return validationIssue('La latitud y la longitud son obligatorias para su tipo de ubicación actual')
      }
      return false
    case 'LONGITUDE':
      const isNotLng = value > 180 || value < -180
      if (isNotLng) {
        return validationIssue('El campo debe ser una longitud válida.')
      }
    ...
    case 'NUMBER':
      ...
      if (isRequiredButEmpty) {
        ...
        return validationIssue('El campo debe ser un número válido')
      } else {
        return false
      }
    case 'DAY_OF_WEEK_BOOLEAN':
      ...
      if (!hasService && name === 'monday') {
        ...
        return validationIssue('El calendario debe tener servicio durante al menos un día')
      }
      return false
    case 'DROPDOWN':
      if (
    ...
    case 'GTFS_AGENCY':
      if (
        ...
        if (valueDoesNotExist) {
          return validationIssue('El campo debe completarse para feeds con más de una agencia.')
        }
      }
      return false
    case 'EXCEPTION_DATE': // a date cannot be selected more than once (for all exceptions)
      ...
      if (entity) {
        ...
        if (scheduleExceptions.map(ex => ex.name).includes(castedScheduleException.name)) {
          const reason = `"${castedScheduleException.name}" ya se usa en otra excepción`
      ...
      for (let i = 0; i < value.length; i++) {
        ...
        if (dateMap[value[i]] && dateMap[value[i]].length > 1) {
          ...
          const reason = `Fecha (${value[
            i
          ]}) no puede aparecer más de una vez para todas las excepciones`
    ...
    case 'POSITIVE_INT':
      ...
      if (
        ...
        return validationIssue('El campo debe ser un entero positivo')
      }
      return false
    case 'POSITIVE_NUM':
