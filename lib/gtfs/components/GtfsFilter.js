...
export default class GtfsFilter extends Component<Props> {
  getFeedSummary (feeds: Array<Feed>) {
    ...
    return feeds.length === 0
      ? 'No se han seleccionado feeds'
      : feeds.length < 3
        ? `Buscando ${feedNames}`
        : `Buscando ${feeds.length} feeds`
  }
        ...
        <Dropdown
          ...
          <Dropdown.Menu className='scrollable-dropdown'>
            {allFeeds.length > 0
              ...
              : <MenuItem disabled>No hay feeds disponibles</MenuItem>
            }
          </Dropdown.Menu>
        </Dropdown>
        <Button
          ...
          {activeFeeds.length > 0 ? 'Eliminar todo' : 'Añadir todo'}
        </Button>
      </ButtonToolbar>
    )
  }
