...
const FS_SUMMARY_URL = '/api/manager/secure/feedsourceSummaries'
/**
...
export function fetchProjectFeeds (projectId: string) {
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    return dispatch(secureFetch(`${FS_SUMMARY_URL}?projectId=${projectId}`))
...
export function onFeedSourceViewerMount (initialProps: any) {
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    } else if (!feedSource || !feedSource.feedVersionSummaries) {
      // Fetch feed source with versions and snapshots if the feed source is not
      // found or if it is a FeedSourceSummary (does not have feedVersionSummaries)
...
export function runFetchFeed (feedSource: Feed) {
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    return dispatch(secureFetch(`${FS_URL}/${feedSource.id}/fetch`, 'post'))
      .then(res => dispatch(handleJobResponse(res, 'Erroeur lors de la récupération du flux')))
  }
}
