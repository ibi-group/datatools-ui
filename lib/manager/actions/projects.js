...
// Also used in deployments.js (actions)
export const receiveDeployments = createAction(
  'RECEIVE_DEPLOYMENTS',
  (payload: {
...
export function updateProject (
  ...
  return async function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    return dispatch(secureFetch(url, 'put', changes))
      .then(async (res) => {
        const project = (getState().projects.all || []).find(p => p.id === projectId)
        if (fetchFeedsAndDeployments) {
          // To expedite UI response, don't refetch the deployment list when updating a project.
          // (The user should refresh the deployments page instead.)
          const deployments = project ? project.deployments || [] : []
