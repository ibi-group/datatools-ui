    ...
    switch (job.type) {
        ...
        if (isExtensionEnabled('mtc')) {
          ...
          if (firstDate && moment(firstDate).isAfter(now)) {
            dispatch(setStatusModal({
              title: 'Advertencia: el Feed contiene servicio solo para fechas futuras!',
              body: `Use el botón 'Merge' para fusionarse con la versión de feed actualmente activa.`
            }))
          }
        }
        break
      ...
      case 'MERGE_FEED_VERSIONS':
        if (job.mergeType === 'REGIONAL') {
          ...
          if (!job.projectId) {
            // FIXME use setErrorMessage instead?
            throw new Error('No se encontró ningún proyecto en el trabajo')
          }
          // FIXME
