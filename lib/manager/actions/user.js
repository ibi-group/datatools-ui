...
export function createPublicUser (credentials: any) {
  window.alert('Esta función ya no se mantiene')
...
export function getRecentActivity (user: ManagerUserState) {
  return function (dispatch: dispatchFn, getState: getStateFn) {
    if (!user.profile) throw new Error('El perfil no existe en el estado del usuario')
...
export function resendEmailConfirmation (user: ManagerUserState): Promise<boolean> {
  ...
  return async function (dispatch: dispatchFn, getState: getStateFn) {
    if (!user.profile || !user.profile.user_id) {
      throw new Error('ID de perfil de usuario faltante!')
    }
...
export function receiveTokenAndProfile (authResult: ?{profile: UserProfile, token: string}) {
  ...
  if (!profile.app_metadata) {
    ...
    if (profile['http://datatools/app_metadata']) {
      ...
      throw new Error('La autenticación de usuario Auth0 no está configurada correctamente!')
    }
  }
  return userLoggedIn({
    token,
  ...
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    if (clientIndex === -1) {
      return dispatch(setErrorMessage({
        message: `No se pudieron encontrar los metadatos del usuario\n${JSON.stringify(user)}`
      }))
    }
    ...
    return dispatch(secureFetch(url, 'put', payload))
      ...
      .then(responseJson => {
        if (status >= 400) {
          return dispatch(setErrorMessage({
            message: `No se pudieron actualizar los metadatos del usuario\n${JSON.stringify(responseJson)}`
          }))
        }
        const {profile} = getState().user
        if (!profile) throw new Error('No se pudo encontrar el perfil de usuario en el estado.')
