      ...
      .then(res => {
        if (res.status === 304) {
          ...
          dispatch(setErrorMessage({
            title: `Advertencia: Versión de Feed para ${feedSource.name} no procesado`,
            message: 'Subida del feed cancelada porque coincide con la última versión del feed.'
          }))
          ...
          res.json()
            .then(json => {
              ...
              dispatch(setErrorMessage({
                message: message || 'Error subiendo Feed source',
                detail
              }))
            })
        } else {
      ...
      .then(data => {
        ...
        if (editor) {
          ...
          if (replaceNew) {
            ...
            if (!feedSourceId || !id) {
              throw new Error('feedSourceId o id no esta definido')
            }
    ...
    return dispatch(secureFetch(url))
      .then(res => {
        ...
        if (res.status === 202) {
          ...
          dispatch(handleJobResponse(res, 'Error al construir la red de transporte'))
          return []
        }
        return res.json()
      })
        ...
        if (getConfigProperty('application.data.use_s3_storage')) {
          if (!json.url) {
            dispatch(setErrorMessage({
              ...
              message: 'No se pudo descargar el feed. Por favor, póngase en contacto con el administrador del sistema.'
            }))
          } else {
...
export function createFeedVersionFromSnapshot (
  ...
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    return dispatch(secureFetch(url, 'post'))
      .then(res => dispatch(handleJobResponse(res, 'Error descargando snapshot')))
  }
}
...
export function exportVersionShapes (feedVersionId: string, type: ShapefileExportType) {
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    return dispatch(secureFetch(url, 'post'))
      .then(res => dispatch(handleJobResponse(res, 'Error al exportar GIS')))
  }
}
