  ...
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    if (!token) {
      return dispatch(setErrorMessage({
        title: `Non autorisé`,
        message: 'Vous devez être connecté pour réaliser cette action.'
      }))
    }
      ...
      .then(res => {
        if (res.status === 304) {
          ...
          dispatch(setErrorMessage({
            title: `Attention: Version du flux ${feedSource.name} non traitée`,
            message: 'Chargement du flux annulé car correspond à la dernière version actuelle du flux.'
          }))
          ...
          res.json()
            .then(json => {
              ...
              dispatch(setErrorMessage({
                message: message || 'Erreur lors du chargement du flux',
                detail
              }))
            })
        } else {
...
export function fetchValidationErrors ({
  ...
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    return dispatch(fetchGraphQL({
      ...
      errorMessage: 'Erreur lors de la récupération des résultats de validation'
    }))
      .then(data => {
        if (data && data.feed) {
    ...
    return dispatch(fetchGraphQL({query, variables: {namespace}}))
      .then(data => {
        if (data.feed) {
          ...
          console.error('Impossible de récupérer les erreurs', data)
        }
      })
      .catch(err => console.log(err))
  }
...
export function fetchFeedVersionIsochrones (
  ...
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    if (!feedVersion) {
      console.warn('Version du flux non définie, impossible de récupérer les versions isochrones')
      return
    }
    ...
    return dispatch(secureFetch(url))
      .then(res => {
        ...
        if (res.status === 202) {
          ...
          dispatch(handleJobResponse(res, 'Erreur lors de la construction du réseau'))
          return []
        }
        return res.json()
      })
        ...
        if (getConfigProperty('application.data.use_s3_storage')) {
          if (!json.url) {
            dispatch(setErrorMessage({
              ...
              message: 'Impossible de télécharger le flux. Contactez votre administrateur système.'
            }))
          } else {
...
export function createFeedVersionFromSnapshot (
  ...
  return function (dispatch: dispatchFn, getState: getStateFn) {
    ...
    return dispatch(secureFetch(url, 'post'))
      .then(res => dispatch(handleJobResponse(res, 'Erreur lors du téléchargement de l’instantané')))
  }
}
...
export function setVersionIndex (
  ...
  return async function (dispatch: dispatchFn, getState: getStateFn) {
    if (feed.feedVersionSummaries) {
      ...
      console.warn('Pas de versions trouvées pour le flux.', feed)
    }
  }
}
