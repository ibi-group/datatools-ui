...
function getVersionDisplayData (
  ...
  if (validationSummary) {
    const errorCount = validationSummary.errorCount || 'Ninguno'
...
function generateFeedSourceLabelRenderVars (feedSource: Feed, comparisonValidationSummary: ?ValidationSummary) {
  ...
  lastVersionUpdate = lastVersionUpdate
    ? `Última actualización hace ${lastVersionUpdate}`
    : ''
...
class FeedInfo extends PureComponent<{ feedSource: Feed, project: Project, user: ManagerUserState }> {
  ...
  render () {
    ...
    const lastUpdateText = feedSource.lastUpdated
      ? `Última actualización ${moment(feedSource.lastUpdated).format(dateFormat)}`
      : 'Sin actualizaciones'
  ...
  render () {
    ...
    return (
      ...
      >
        <ConfirmModal
          ref='deleteModal'
          title='¿Eliminar Feed Source?'
          body={`¿Está seguro de que desea eliminar el Feed Source ${feedSource.name}?`}
          onConfirm={this._onConfirmDelete}
        />
        <SelectFileModal
          ref='uploadModal'
          title='Cargar Feed'
          body='Seleccione un feed GTFS para cargar:'
          onConfirm={this._onConfirmUpload}
          errorMessage='El archivo cargado debe ser un archivo zip válido (.zip).'
        />
        <MenuItem
          disabled={editGtfsDisabled}
          onClick={this._onClickEdit}
        >
          <Glyphicon glyph='pencil' /> Abrir en el editor
        </MenuItem>
        <MenuItem
        ...
        {isModuleEnabled('deployment')
          ? <MenuItem
            ...
            title={disabled
              ? 'No tienes permiso para despolegar Feeds'
              : !feedSource.deployable
                ? 'Feed Source no es desplegable. Cambialo en la configuración del Feed Source.'
                : !feedSource.latestValidation
                  ? 'No existen versiones. Crear una nueva versión para desplegar el Feed'
                  : 'Desplegar feed source.'
            }
            onClick={this._onClickDeploy}
          >
            <Glyphicon glyph='globe' /> Desplegar
          </MenuItem>
          : null
        }
        ...
        >
          <Glyphicon glyph='link' /> Ver página pública
        </MenuItem>
        <MenuItem divider />
        <MenuItem
