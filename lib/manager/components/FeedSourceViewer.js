...
export default class FeedSourceViewer extends Component<Props> {
  ...
  confirmDeleteFeedSource = () => {
    ...
    this.refs.page.showConfirmModal({
      ...
      body: 'Êtes-vous sûr de vouloir supprimer ce flux ? Cette action est irréversible et toutes les versions seront supprimées.',
      ...
      return (
        <ManagerPage ref='page'>
          <Grid fluid>
            <Row>
              <Col xs={12}>
                <p>Pas de flux trouvé pour <strong>{this.props.feedSourceId}</strong></p>
                <p><Link to='/project'>Retourner à la liste des projets</Link></p>
              </Col>
            </Row>
          </Grid>
        </ManagerPage>
