...
export default class ProjectViewer extends Component<Props, State> {
  ...
  componentDidUpdate () {
    ...
    if (
      ...
      activeComponent === 'deployments' &&
      // Wait for project data to be available first.
      !isFetching &&
      // All deployment summaries have not been fetched.
      !this.state.deploymentsFetched
    ) {
      if (!activeSubComponent) {
        // The user is looking at the deployments list.
        fetchProjectDeployments(projectId)
        this.setState({ deploymentsFetched: true })
      } else if (!(project.deployments || []).find(d => d.id === activeSubComponent)) {
        // The desired deployment has not been fetched.
        fetchDeployment(activeSubComponent)
      }
    }
  }
