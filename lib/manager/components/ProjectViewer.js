  ...
  _renderPublicFeeds = () => {
    ...
    return isModuleEnabled('enterprise') && !this._isProjectEditDisabled() &&
      <div style={{marginBottom: '20px'}}>
        ...
        {s3Bucket &&
          <p className='small'>
            <strong>Nota:</strong> Puedes consultar los Feeds públicos{' '}
            <a target='_blank' href={publicFeedsLink}>aqui</a>.
          </p>
        }
      </div>
  }
          ...
          <Grid fluid>
            <Row>
              <Col xs={12}>
                ...
                No se ha encontrado ningún proyecto para <strong>{this.props.projectId}</strong>
                </p>
                <p><Link to='/project'>Volver a la lista de proyectos</Link></p>
              </Col>
            </Row>
          </Grid>
        </ManagerPage>
...
class ProjectSummaryPanel extends Component<{feedSources: Array<Feed>, project: Project}> {
  render () {
    ...
    const serviceSeconds = feedSources
      ...
      .reduce((a, b) => a + b, 0)
    return (
      <Panel header={<h3>{project.name} resumen</h3>}>
        <ListGroup fill>
          <ListGroupItem>Número de Feeds: {feedSources.length}</ListGroupItem>
          <ListGroupItem>Errores: {errorCount}</ListGroupItem>
          <ListGroupItem>
            Servicio Total:{' '}
...
const ExplanatoryPanel = ({ project }) => {
  ...
  if (project.labels.length <= 3) {
    return (
      <Panel header={<h3>¿Qué es un Feed Source?</h3>}>
        Un Feed Source define la ubicación de un Feed GTFS.
        Un GTFS se puede popular a través de la Actualización automática, editando o cargando directamente
        un archivo zip.
      </Panel>
    )
  }
