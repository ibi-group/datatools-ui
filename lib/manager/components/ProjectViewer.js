...
export default class ProjectViewer extends Component<Props, State> {
  ...
  componentDidUpdate () {
    const { activeComponent, isFetching, projectId } = this.props
    if (isModuleEnabled('deployment') && activeComponent === 'deployments' && !isFetching && !this.state.deploymentsFetched) {
      this.props.fetchProjectDeployments(projectId)
