...
export default class CurrentDeploymentPanel extends Component<Props, State> {
  ...
  render () {
    ...
    return (
      <Panel header={<h3><Icon type='server' /> Resumen de despliegue</h3>}>
        {deployJob &&
          <div>
            Despliegue en progreso...<br />
        ...
        {hasPreviouslyDeployed
          ...
          : 'No se ha encontrado ningún despliegue actual'
        }
        {ec2Info // If has EC2 info, show EC2 instances box.
          ? <div>
            <h4>
              EC2 instancias ({getActiveInstanceCount(deployment.ec2Instances)} activas)
              <Button
                bsSize='xsmall'
...
class DeployJobSummary extends Component<{
  ...
  render () {
    ...
    let deploymentType = 'desconocido'
    if (server) {
      deploymentType = server.ec2Info
        ? 'ELB'
        : server.internalUrl
          ? 'Construir grafo sobre wire'
          : server.s3Bucket
            ? 'solo S3'
            : 'desconocido'
    }
    ...
    return (
      <div>
        <ul className='list-unstyled'>
          <li>
            Desplegado en: {serverLabel}
          </li>
          <li>Estado: {summary.status.error ? 'Failure' : 'Success'}</li>
          <li>Tipo: {deploymentType}</li>
          <li>Tiempo de despliegue: {formatTimestamp(summary.finishTime)}</li>
          {summary
            ? <li>otp versión: {summary.otpVersion}</li>
            : null
          }
        </ul>
        <ButtonToolbar>
          ...
          <Button
            ...
            <Icon type='file-text-o' /> Registro de compilación
          </Button>
          <Button
            bsSize='xsmall'
            onClick={this._onClickDownloadReport}>
            <Icon type='file-zip-o' /> Reporte de compilación del grafo
          </Button>
          <Button
            bsSize='xsmall'
            onClick={this._onClickDownloadBundle}>
            <Icon type='file-zip-o' /> Paquete
          </Button>
        </ButtonToolbar>
