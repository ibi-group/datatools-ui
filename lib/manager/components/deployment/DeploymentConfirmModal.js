  ...
  render () {
    ...
    return (
      <Modal show onHide={this._onClose}>
        ...
        <Body>
          <h3>Configuración de despliegues</h3>
          <ul className='list-unstyled'>
            <li>
              <Glyphicon glyph='fullscreen' />{' '}Límites de OSM:{' '}
              {isMissingBounds
            ...
            <li>
              <Glyphicon glyph='list' />{' '}Desplegando {deployment.feedVersions.length} feeds:{' '}
              {getFeedNames(deployment.feedVersions)}
            </li>
          </ul>
          <h3>Configuración de OpenTripPlanner</h3>
          <ul className='list-unstyled'>
            <li data-test-id='deployment-router-id'>
              ID del enrutador: {deployment.routerId || '[por defecto]'}
            </li>
            <li>Configuración de compilación: {deployment.customBuildConfig ? 'personalizado' : 'por defecto'}</li>
            <li>Configuración del enrutador: {deployment.customRouterConfig ? 'personalizado' : 'por defecto'}</li>
          </ul>
