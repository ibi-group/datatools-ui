...
export default class DeploymentViewer extends Component<Props, State> {
  ...
  _incrementAllVersionsToLatest = () => {
    ...
    const message = `¿Estás seguro de que te gustaría avanzar ${feedsNeedingUpdates.length} feed(s) a sus últimas versiones?`
    ...
    return (
      <div>
            ...
            <DropdownButton
              ...
              title={
                <span>
                  ...
                  <span className='hidden-xs'> Exportar (.shp)</span>
                </span>
              }>
              <MenuItem eventKey='STOPS'><Icon type='map-marker' /> Paradas</MenuItem>
              <MenuItem eventKey='ROUTES'><Icon type='ellipsis-h' /> Rutas</MenuItem>
            </DropdownButton>
            <DropdownButton
              bsStyle='primary'
              ...
              <LinkContainer
                ...
                key={project.id}>
                <MenuItem>
                  <Icon type='cog' /> Administrar servidores
                </MenuItem>
              </LinkContainer>
            </DropdownButton>
          </ButtonGroup>
        ...
        <small title={deployment.lastDeployed && formatTimestamp(deployment.lastDeployed)}>
          <Icon type='clock-o' />
          {' '}
          Último despliegue {deployment.lastDeployed ? fromNow(deployment.lastDeployed) : na}
        </small>
      </div>
    )
  }
