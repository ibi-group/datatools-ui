...
export default class FeedTransformRules extends Component<TransformRulesProps> {
  ...
  _removeRuleSet = () => {
    ...
    const deleteConfirmed = window.confirm(
      `¿Está seguro de que desea eliminar la transformación ${index + 1}?`
    )
    ...
    return (
      <ListGroupItem className={ruleSet.active ? '' : 'disabled'}>
        <h4>
          <ButtonToolbar className='pull-right'>
            <Button
              ...
              {ruleSet.active
                ? <span><Icon type='pause' /> Pausar</span>
                : <span><Icon type='play' /> Resumir</span>
              }
            </Button>
            <Button
              bsSize='xsmall'
          ...
          <Icon type='wrench' /> Transformación {index + 1}{' '}
          {!ruleSet.active ? '(Pausado) ' : ''}
          <small>{methodBadges}</small>
        </h4>
        <small>
          Aplique esta transformación a los feeds GTFS creados a través de los siguientes métodos.
        </small>
        <Select
          style={{margin: '5px 0px 10px 0px'}}
          clearable={false}
          multi
          placeholder='Seleccionar métodos de obtención del GTFS'
        ...
        {ruleSet.transformations.length > 0
          ...
          : <div className='lead'>No se han definido pasos de transformación.</div>
        }
        <ButtonToolbar>
          <DropdownButton
            title='Agregar paso a la transformación'
