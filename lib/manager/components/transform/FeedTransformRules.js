...
export default class FeedTransformRules extends Component<TransformRulesProps> {
  ...
  _removeRuleSet = () => {
    ...
    const deleteConfirmed = window.confirm(
      `Êtes-vous sûr de vouloir supprimer la Transformation ${index + 1}?`
    )
  ...
  render () {
    ...
    return (
      <ListGroupItem className={ruleSet.active ? '' : 'disabled'}>
        <h4>
          <ButtonToolbar className='pull-right'>
            <Button
              ...
              {ruleSet.active
                ? <span><Icon type='pause' /> Mettre en pause</span>
                : <span><Icon type='play' /> Reprendre</span>
              }
            </Button>
            <Button
              bsSize='xsmall'
              onClick={this._removeRuleSet}>
              <Icon type='trash' /> Supprimer
            </Button>
          </ButtonToolbar>
        ...
        <small>
          Appliquer cette transformation à tous les flux GTFS créés par les méthodes suivantes.
        </small>
        <Select
        ...
        {ruleSet.transformations.length > 0
          ...
          : <div className='lead'>Pas d'étapes de transformation définies.</div>
        }
        <ButtonToolbar>
          <DropdownButton
