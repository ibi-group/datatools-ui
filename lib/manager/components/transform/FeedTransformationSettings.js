...
export default class FeedTransformationSettings extends Component<Props> {
  ...
  render () {
    ...
    if (disabled) {
      return (
        <p className='lead'>
          El usuario no está autorizado para modificar la configuración de los Feed transformations.
        </p>
      )
    }
    return (
      <Col xs={7}>
        {/* Settings */}
        <Panel header={<h3>Ajustes de las transformaciones</h3>}>
          <ListGroup fill>
            <ListGroupItem>
              <p>
                Los Feed Transformations brindan una manera de transformar automáticamente 
                los datos GTFS que se cargan en Herramientas de datos. Agregue una transformación, 
                describa cuándo debe aplicarse (por ejemplo, solo a los feeds cargados manualmente)
                y luego defina una serie de pasos para modificar los datos.
              </p>
