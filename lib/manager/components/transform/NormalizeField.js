  ...
  render () {
    ...
    const normalizeTooltip = (
      <Tooltip id='normalize-space-tooltip'>
        <p>
          Si está marcada, esta opción agregará o reducirá el espacio que rodea
          el texto a sustituir a un espacio.
        </p>
        <p>
          Por ejemplo, si "@" fuera reemplazada por "y",
          entonces "12th@&nbsp;&nbsp;&nbsp;West" se convertiría en "12th y West".
        </p>
      </Tooltip>
    )
    ...
    const substituteTooltip = (
      <Tooltip id='substitute-space-tooltip'>
        <p>
          Ingrese el texto a buscar como una expresión regular (regex).
          Es posible que sea necesario escapar algunos caracteres (por ejemplo, ingrese "\+" para el símbolo "+").
        </p>
        <p>
          Haga clic en el <Icon title='Prueba esta expresión regular' type='flask' /> icono junto a una expresión regular a continuación para probarlo
          o para obtener ayuda para escribir uno.
        </p>
      </Tooltip>
    )
    ...
    return (
      <div>
        ...
        >
          Hacer cumplir el caso del título (por ejemplo, "Main Street y First Avenue")
        </Checkbox>
        <FormGroup>
          <ControlLabel htmlFor={capitalizationExceptionListId}>
            Excepciones de uso de mayúsculas (por ejemplo, "a través de" o "MLK")
          </ControlLabel>
          <Select.Creatable
            arrowRenderer={null}
            clearable={false}
            ...
            placeholder='Ingrese las excepciones y luego presione enter'
          ...
          <tbody>
            ...
            <tr>
              <td colSpan='4' style={{ borderTop: 'none' }}>
                {substitutions.length === 0 && <p>(No se han definido sustituciones)</p>}
                <Button
                  bsSize='xsmall'
