...
export default class SubstitutionRow extends Component<Props, Substitution> {
  ...
  render () {
    ...
    if (isPatternBlank) {
      validationMessage = 'El patrón de búsqueda de sustitución no puede estar vacío'
    } else if (isPatternInvalid) {
      validationMessage = `El patrón de búsqueda por sustitución '${originalSubstitution.pattern}' es invalido`
    }
          ...
          <FormGroup style={{ margin: 0 }} validationState={isEditingInvalid ? 'error' : null}>
            <InputGroup>
              <FormControl
                ...
                placeholder='Texto (expresión regular) para encontrar'
                readOnly={!isEditing}
              ...
              <InputGroup.Addon style={patternStyleOverride}>
                <a
                  href={regexTestLink}
                  target='_blank'
                  title='Prueba esta expresión regular'
                >
                  <Icon type='flask' />
                </a>
              </InputGroup.Addon>
            ...
            ? (
              ...
              <ButtonGroup style={buttonGroupStyle}>
                <Button
                  ...
                  title={isPatternInvalid ? 'No se puede guardar, hay errores en esta fila.' : 'Guardar cambios en esta fila'}
                >
                  <Icon type='check' />
                </Button>
                <Button
