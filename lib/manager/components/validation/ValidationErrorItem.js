...
export default class ValidationErrorItem extends Component<Props, State> {
  ...
  _onEditClick = () => {
    ...
    if (component) {
      ...
      console.warn(`Impossible de lier à l’éditeur.`, error)
    }
  }

  _formatValue = (value: any) => {
    return value === null
      ? <span className='text-light'>N/D</span>
      : value
  }
        ...
        <td>
          <Button
            ...
            title={noFeedInEditor
              ? 'Importer le flux dans le GTFS pour résoudre l’erreur'
              : typeIsNotEditable
                ? 'Erreur non résoluble dans l’éditeur'
                : 'Fix in editor'}
          ...
          <Button
            ...
            disabled={noEntityId}
            title={noEntityId ? `Pas d’entité à afficher` : `Voir les détails: ${type}`}
            style={{margin: 0, padding: 0}}
            bsStyle='link'
          >
            <Icon type='info' />
