...
export default class FeedVersionDetails extends Component<Props> {
  ...
  _mergeItemFormatter = (v: FeedVersionSummary, activeVersion: ?FeedVersionSummary) => {
    ...
    if (v.retrievalMethod === 'SERVICE_PERIOD_MERGE') {
      name = '(No se puede volver a fusionar el feed)'
      disabled = true
    }
    if (activeVersion && v.id === activeVersion.id) {
      name = '(No puede fusionarse con uno mismo)'
      disabled = true
    }
    return (
      <MenuItem
  ...
  render () {
    ...
    const mergeButtonLabel = isMergedServicePeriods
      ? 'No se puede volver a fusionar el feed'
      : 'Fusionar con la versión'
    ...
    if (hasMtcExtension) {
      if (hasBlockingIssue || hasGtfsPlusBlockingIssue) {
        publishWarningMessage = (
          <span>
            No se puede publicar la versión porque tiene un{' '}
            {hasGtfsPlusBlockingIssue ? 'GTFS+ ' : ''}
            problema de bloqueo.
            (See{' '}
            <Link
              to={`/feed/${feedSource.id}/version/${version.version}/${hasGtfsPlusBlockingIssue ? 'gtfsplus' : 'issues'}`}>
              {hasGtfsPlusBlockingIssue ? 'GTFS+' : 'validation'} asuntos
            </Link>.)
          </span>
        )
      } else if (feedSource.autoPublish) {
        publishWarningMessage = 'Recordatorio: ¡Este feed ya está configurado para publicarse automáticamente después de la recuperación automática!'
      }
    }
        ...
        {hasMtcExtension &&
          <ButtonToolbar className='pull-right' style={{ marginTop: '2px' }}>
            ...
            <Button
              ...
              bsStyle={isPublished ? 'éxito' : 'advertencia'}
              onClick={this._onClickPublish}>
              {isPublished
                ? <span><Icon type='check-circle' /> Publicado</span>
                : processing
                  ? <span>Procesando...</span>
                  : <span>Publicar a MTC</span>
              }
            </Button>
          </ButtonToolbar>
        }
        ...
        <p style={{clear: 'both'}}>
          ...
          {summary && summary.bounds && getConfigProperty('application.dev')
            ? <span>
              <Icon type='globe' />
              {' '}
              {this.getBoundsArea(summary.bounds)} metros cuadrados
            </span>
            : null
          }
        </p>
