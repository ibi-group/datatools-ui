...
function isValidLatitude (n: number): {
  ...
  if (n <= 90 && n >= -90) {
    ...
    return {
      error: `Se esperaba un valor entre 90 y -90, pero se encontró ${n}`,
      valid: false
    }
  }
}
...
function isValidLongitude (n: number): {
  ...
  if (n <= 180 && n >= -180) {
    ...
    return {
      error: `Se esperaba un valor entre 180 y -180, pero se encontró ${n}`,
      valid: false
    }
  }
}
...
export function parseBounds (val: string): {
  ...
  function validateVal (val, label, validationFn) {
    ...
    if (!validationResult.valid) {
      errors.push(`${label} valor no es válido. ${validationResult.error || ''}`)
    }
  }
  ...
  if (bBox.length !== 4) {
    return {
      errors: [`Cantidad incorrecta de valores. Encontrados ${bBox.length}, esperados 4.`],
      valid: false
    }
  }
  ...
  validateVal(west, 'Oeste', isValidLongitude)
  validateVal(east, 'Este', isValidLongitude)
  validateVal(north, 'Norte', isValidLatitude)
  validateVal(south, 'Sur', isValidLatitude)
