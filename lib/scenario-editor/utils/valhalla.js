...
export async function polyline (
  ...
  try {
    ...
    for (let i = 0; i < j; i += chunk) {
      ...
      json = await routeWithGraphHopper(chunkedPoints, avoidMotorways, followOption)
...
export function routeWithGraphHopper (
  ...
  if (process.env.GRAPH_HOPPER_ALTERNATES) {
    ...
    alternates.forEach(alternative => {
      ...
      if (arePointsValid(points, BBOX)) {
        ...
        // add more followOptions here if needed
      }
    })
  }
  ...
  const graphHopperRequest = avoidMotorways && followOption === 'STREET'
